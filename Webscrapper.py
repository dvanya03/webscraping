...
requests = 0
destinations = ['HYD']
startdates = ['2023-06-01', '2023-06-15']
for destination in destinations:
    for startdate in startdates:
        requests = requests + 1
        while scrape('DEL', destination, startdate, 3, requests) != "success":
            requests = requests + 1
...
if heatmap_results.size != 0:
    sns.heatmap(heatmap_results, annot=True, annot_kws={"size": 24}, fmt='.0f', cmap="RdYlGn_r")
    plt.show()
else:
    print("No flight data available for the specified destination-start date combinations.")
